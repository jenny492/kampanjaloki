...
router.get('/one/:id', (req, res) => {
    ...
    db.get('select * from campaigns where campaignid = ?', [id], (error, result) => {
        if (error) {
            console.log(error.message);
            return res.status(400).json({ message: error.message });
        }
...
router.get('/imageurl', (req, res) => {
    db.all('select image_url from campaigns where image_url IS NOT NULL', (error, result) => {
        if (error) {
            console.log(error.message);
            return res.status(400).json({ message: error.message });
        }
...
router.delete('/delete/:id', (req, res) => {
    ...
    db.run('delete from campaigns where campaignid = ?', [id], function (error) {
        if (error) {
            console.log(error.message);
            return res.status(400).json({ message: error.message });
        }
...
router.post('/add', upload.single('kuva'), (req, res) => {
