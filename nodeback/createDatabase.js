...
db.serialize(() => {
  ...
  db.run(`
    CREATE TABLE IF NOT EXISTS campaigns (
      ...
      created_at   TEXT,
      owner_id     INTEGER
      -- FOREIGN KEY (owner_id) REFERENCES users(id) -- jos joskus lisäät users-taulun
    )
  `);
  ...
  // Tyhjennä taulut (oikeassa FK-järjestyksessä)
  ...
  const insertCampaign = db.prepare(`
    INSERT INTO campaigns (id, name, description, image_url, created_at, owner_id)
    VALUES (?, ?, ?, ?, ?, ?)
  `);
  ...
  const insertCharacter = db.prepare(`
    INSERT INTO characters (id, name, description, image_url, link, owner_id)
    VALUES (?, ?, ?, ?, ?, ?)
  `);
  ...
  const insertSession = db.prepare(`
    INSERT INTO sessions (id, campaign_id, title, content, session_date, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  const insertEvent = db.prepare(`
    INSERT INTO events (id, session_id, title, order_index, content, details)
    VALUES (?, ?, ?, ?, ?, ?)
  `);
